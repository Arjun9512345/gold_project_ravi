...
@Component({
  selector: 'app-sale_history_all_store_List_page',
  templateUrl: './sale_history_all_store.component.html',
  styleUrls: ['./sale_history_all_store.component.scss']
})
export class Sale_history_all_storeComponent implements OnInit {

  store_code:string='';
  order_type:string='';
  storeMasterListModels: Array<StoreMasterListModel> = [];
  displayedColumns: string[] = ['sale_datetime', 'sale_header_no', 'parent_so_no','store_id', 'cust_details','cust_phone_no',
  ...
  constructor(public sessionManageMent: SessionManageMent,
    ...
    try {
      let paggingData: PaggingModel = this.sessionManageMent.getSaleHistoryAllPagging;
  ...
  applyFilter(filterValue: string, keyName: string) {
    ...
    else if(keyName=='sale_header_no')
      this.sale_header_no=filterValue;
    else if(keyName=='parent_so_no')
      this.parent_so_no=filterValue;
    else if(keyName=='cust_details')
      this.customer_detail=filterValue;
    else if(keyName=='cust_phone_no'){
      this.router.navigateByUrl('/pointofsale/sale_history_all');
      this.cust_phone_no=filterValue;
    }
    else if(keyName=='store_id')
      this.store_code=filterValue;
  ...
  clickOnRow(sale_header_no: string) {
    ...
    if (sale_header_no == null || sale_header_no == undefined || sale_header_no == '') {
      paggingData = this.sessionManageMent.getSaleHistoryAllPagging;
      if (paggingData == null || paggingData == undefined) {
    ...
    this.viewBrandDataSource.data.map(item => {
      if ((sale_header_no == item.sale_header_no || paggingData.primary_value == item.sale_header_no) && paggingData.page_number == this.PageNumber) {
        this.sessionManageMent.setSaleHistoryAllPagging({
  ...
  download_Excel(start_date,end_date) {
    if(!this.loadding){
      this.loadding=true;
      this.webApiHttp.Post(this.webApiHttp.ApiURLArray.GetAllSaleRange,{
        start_date:start_date,
        end_date:end_date
      }).then(result => {
        if(isArray(result) && result.length>0){
          this._toster.error(result[0].message, 'Error');
        }else if(result.condition.toLowerCase()=='true'){
          this.excelService.MultisheetWorkbook([result.sale_header,result.sale_lines,result.sale_payment,result.sale_credit_note],
            ['sale_header','sale_lines','sale_payment','sale_credit_note'], 'Sale_Report');
        }
      }).finally(() => {
        this.loadding=false;
      });
