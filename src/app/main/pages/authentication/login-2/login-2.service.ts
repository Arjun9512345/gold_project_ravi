...
export class Login2Service implements Resolve<any> {
  ...
  getLoginWindowByTokenAccess(postedjson: any): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.WebApiHttp.Post(this.WebApiHttp.ApiURLArray.LoginWindowsByToken, postedjson).then(result => resolve(result), reject)
        .catch(error => reject(error));
    });
  }
}
