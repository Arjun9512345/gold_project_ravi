...
@Component({
  selector: 'app-approvecycle_count_list_main_Page',
  templateUrl: './approvalcycle_count_list.component.html',
  styleUrls: ['./approvalcycle_count_list.component.scss']
})
export class Approvalcycle_count_listComponent implements OnInit {
  ...
  cyclecount_list(qery_parameter: string) {
    try {
      ...
      this.webApiHttp.Post(this.webApiHttp.ApiURLArray.ApproveGetCycleCountList, json).then(
        result => {
  ...
  viewinfo(element: CycleCountListModel) {
    this.router.navigate(['/internal/approvalCyclepage', {res: this._encriptDecript.encrypt(element.document_no) }])
  }
