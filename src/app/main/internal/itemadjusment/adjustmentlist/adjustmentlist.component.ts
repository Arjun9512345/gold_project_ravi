...
export class AdjustmentlistComponent implements OnInit {
  ...
  displayedColumns: string[] = ['adjustment_no', 'adj_type', 'approval_status', 'created_by', 'created_on', 'Update', 'View'];
  ...
  applyFilter(filterValue: string, keyName: string) {
    ...
    for (let i = 0; i < this.filter_dynamic.length; i++) {
      if (this.filter_dynamic[i].filter_value != '') {
        if (i == 0)
          query = ' and [ah].' + this.filter_dynamic[i].filterKey + " like '%" + this.filter_dynamic[i].filter_value + "%'" + (i == this.filter_dynamic.length - 1 ? '' : ' AND ')
        else
